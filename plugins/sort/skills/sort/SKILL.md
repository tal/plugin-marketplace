---
name: sort
description: Sort and organize an accumulating folder of files — defaults to the folder Claude Code was launched in (commonly `~/Downloads`, but works equally on `~/Desktop`, a project inbox, or any directory). Trigger when the user runs `/sort` (with optional path/glob), or asks to "sort my downloads", "organize this folder", "clean up the desktop", "tidy this directory", "categorize these files", "deal with everything in here", or any phrasing that implies a folder needs tidying — even when they don't say "sort". Also trigger when the user complains about accumulated DMGs, zips, PDFs, screenshots, return labels, receipts, app installers, or images piling up somewhere, or wants to file things into an AI Library. Dispatcher that classifies each item — videos, images (vision + OCR identifies returns / receipts / memes / screenshots / photos), archives, disk images, app bundles, documents — and routes to `<target>/AI Library/<topic>/`, created automatically. Videos delegate to `/sort-videos`; documents go through the `documents.md` sub-dispatcher; everything else is handled inline. Auto-deletes installers whose app is already in `/Applications/`; otherwise never deletes without confirmation. User overrides via `sort.md` / `sort.local.md` rule files at project and user scope. Pass a path or glob to process specific items instead of the whole folder.
user-invocable: true
context: fork
allowed-tools:
  - Read
